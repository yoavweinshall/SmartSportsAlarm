...
class DbFutureMatchesService:
    ...
    async def add_future_matches_to_db():
        ...
        supported_competitions_id = [
            str(competition_id) for competition_id in CacheService.get_instance().supported_competition_ids.keys()
        ]
        ...
        await CacheService.get_instance().batch_sync_teams_competitions_links(future_matches)
        await DbFutureMatchesService.write_matches_to_db(future_matches)

    @staticmethod
    async def _get_existing_matches() -> dict[int, dict[str, Any]]:
        matches_in_db = (
            await get_supabase().table("matches").select("external_api_id", "status_text", "start_time").execute()
        )
        return {
            rec["external_api_id"]: {
    ...
    @classmethod
    async def write_matches_to_db(cls, future_matches: list[BaseMatch]) -> None:
        matches_in_db = await DbFutureMatchesService._get_existing_matches()
        matches_to_update = [
            MatchProcessService.process_single_match(match).model_dump(mode="json", exclude_none=True)
            for match in future_matches
            if cls._need_update(match, matches_in_db.get(match.external_api_id))
        ]
        await get_supabase().table("matches").upsert(matches_to_update, on_conflict="external_api_id").execute()
